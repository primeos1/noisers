...
// Get all available teams for this match
...
$available_teams = $teams_stmt->fetchAll();

// Process teams to get player count
foreach ($available_teams as &$team) {
    $player_ids = json_decode($team['players'], true);
    $team['player_count'] = is_array($player_ids) ? count($player_ids) : 0;
}
unset($team);
    ...
    if ($team1_id && $team2_id && $team1_id != $team2_id) {
        try {
            $update_stmt = $pdo->prepare("
                ...
                WHERE id = ?
            ");
            $update_stmt->execute([$team1_id, $team2_id, $set_id]);
            header("Location: match_detail.php?id=$match_id&tab=sets");
            exit();
        } catch (PDOException $e) {
            $error = "Error assigning teams: " . $e->getMessage();
        }
    } else {
...
<html lang="en">
<head>
    <title>Assign Teams to Set — Noisers Football Pro</title>
...
<div class="page-shell">
    ...
    <main class="main-area">
        ...
        <?php if (count($available_teams) >= 2): ?>
        <form method="POST">
            ...
            <div style="display:flex;gap:8px;justify-content:flex-end;">
                <a href="match_detail.php?id=<?php echo $match_id; ?>&tab=sets" class="btn btn-ghost btn-sm">Cancel</a>
                <button type="submit" class="btn btn-primary btn-sm">Assign Teams</button>
            </div>
        </form>
        <?php else: ?>
        <div class="empty-state">
            <p>Not enough teams. Create at least 2 teams first.</p>
            <a href="randomize_sets.php?match_id=<?php echo $match_id; ?>" class="btn btn-primary btn-sm" style="margin-top:14px;">Create Teams</a>
        </div>
        <?php endif; ?>
    </main>
</div>
